...
def analyze_tone(iq, fs, nco_hz, dc_guard_hz=50.0, bp_halfwidth_hz=1000.0):
    """
    Treat `iq` (complex baseband) as one sine wave and measure its quality.

    Returns a dict of metrics; also intended to be printed by report_tone().
    """
    ...
    # ── 1. Coarse center frequency from the FFT peak (DC-guarded) ────────────
    Z     = np.fft.fft(iq)
    freqs = np.fft.fftfreq(n, dt)
    mag   = np.abs(Z)
    mag_g = mag.copy()
    mag_g[np.abs(freqs) < dc_guard_hz] = 0.0
    k        = int(np.argmax(mag_g))
    bin_hz   = fs / n
    f0_fft   = float(freqs[k]) + _parabolic_peak(mag, k) * bin_hz

    # ── 2. Brick-wall band-pass around the tone to isolate it ────────────────
    mask  = (freqs >= f0_fft - bp_halfwidth_hz) & (freqs <= f0_fft + bp_halfwidth_hz)
    iq_bp = np.fft.ifft(Z * mask)
...
def report_tone(m):
    ...
    print(f"  1-s blocks used : {m['n_sec_blocks']}")
    print("-" * 64)
    print(f"  Center freq     : {m['f_center_rf']/1e6:.6f} MHz "
          f"(baseband {m['f_center_bb']:+.3f} Hz)")
    print(f"  Magnitude       : {m['rms']:.5f} rms  /  {m['peak']:.5f} peak  "
          f"({m['rms_dbfs']:+.2f} dBFS)")
    print(f"  Phase jitter    : {m['phase_jitter_deg']:.4f} deg rms "
          f"({m['phase_jitter_rad']*1e3:.3f} mrad)  "
          f"[in +/-{m['bp_halfwidth']/1e3:.1f} kHz]")
    print(f"  Freq drift/sec  : avg {m['drift_avg']*1e3:.3f} mHz/s   "
          f"max {m['drift_max']*1e3:.3f} mHz/s")
    print(f"  Overall drift   : {m['overall_drift']*1e3:+.3f} mHz/s (linear trend)")
    if len(m["blk_freqs"]) >= 2:
        fr = m["blk_freqs"]
        per_sec = "  ".join(f"{f:+.3f}" for f in fr)
        print(f"  Per-second freq : {per_sec}  Hz (baseband)")
    print("=" * 64 + "\n")
...
class ToneSpectrum(LiveSpectrum):
    ...
    def __init__(self, *args, on_grab=None, **kwargs):
        ...
        # TxPwr — click-to-edit text box (0–15), replaces the slider.
        ...
        for tb in (self._txt_txp, self._txt_lna, self._txt_freq):
            try:
        ...
        Y, H = 0.045, 0.05
        # Left
        self._txt_txp.ax.set_position(    [0.045,  Y, 0.05,  H])
        self._txt_lna.ax.set_position(    [0.120,  Y, 0.05,  H])
        ...
        self._radio_rbw.ax.set_position(  [0.850,  0.04, 0.10, 0.09])

        # Text-box labels above the box (centred) instead of to the left.
        for tb, name in ((self._txt_txp, "TxPwr"), (self._txt_lna, "LNA Gn"),
                         (self._txt_freq, "MHz")):
        ...
        self._rbw_label.set_position((0.90, 0.135))
...
def _tx_thread_fn(sock, ip, cc_holder, start_seq):
    ...
    while _tx_running.is_set():
        ...
        if _tone_on.is_set():
            I_tx = (TX_AMPLITUDE * np.cos(phase + phase_step * local_t) * 32767.0).astype(np.int16)
            Q_tx = (TX_AMPLITUDE * np.sin(phase + phase_step * local_t) * 32767.0).astype(np.int16)
        else:
            I_tx = zeros
            Q_tx = zeros
...
def _do_grab(sock, spectrum, seconds=GRAB_SECONDS, outfile=GRAB_FILE):
    ...
    try:
        spectrum._set_status(
            f"Grab done: {metrics['f_center_rf']/1e6:.5f} MHz, "
            f"jitter {metrics['phase_jitter_deg']:.3f} deg, "
            f"drift {metrics['drift_avg']*1e3:.2f} mHz/s  (see console)")
