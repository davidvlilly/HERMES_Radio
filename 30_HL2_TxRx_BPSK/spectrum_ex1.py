...
class LiveSpectrum:
    ...
    def __init__(self, nco_hz, fs, dbfs_ref=0.0,
        ...
        # Matched-filter (correlation) state
        self._corr_mag     = None
        self._corr_df      = 0.0
        self._corr_peak    = 0.0
        self._corr_noise   = 1.0
        self._corr_dropped = 0
        self._corr_locked  = False
        ...
        # Two stacked plots (spectrum top, matched-filter bottom) + controls below
        self.fig, (self.ax, self.ax_corr) = plt.subplots(2, 1, figsize=(11, 8))
        self.fig.subplots_adjust(bottom=0.20, top=0.95, hspace=0.32)

        try:
            self.fig.canvas.manager.set_window_title("HL2 PSK Tx/Rx")
        except Exception:
            pass
        ...
        self.line,     = self.ax.plot([], [], color="#00ff88", linewidth=0.9,
                                      label=f"RBW {rbw_hz:.0f} Hz")
        self.peak_line = self.ax.axvline(nco_hz / 1e6, color="#ff4444",
                                         linewidth=0.8, linestyle="--",
                                         visible=False)
        self.ax.legend(loc="upper right", fontsize=8)

        # Forward-power (PEP) readout — top-left of the spectrum plot
        self._pwr_text = self.ax.text(
            0.01, 0.98, "PEP  --  W   (Fwd ADC --)",
            transform=self.ax.transAxes, color="#ffcc00", fontsize=11,
            va="top", ha="left", family="monospace")

        # ── Matched-filter correlation — bottom plot ──────────────────
        self.ax_corr.set_xlabel("code time  (ms)")
        self.ax_corr.set_ylabel("corr  (dB / noise)")
        self.ax_corr.set_title("Matched filter — waiting ...")
        self.ax_corr.grid(True, color="#333333", linewidth=0.5)
        self.corr_line, = self.ax_corr.plot([], [], color="#ffdd00", linewidth=0.9)
        self._corr_text = self.ax_corr.text(
            0.01, 0.98, "", transform=self.ax_corr.transAxes,
            color="#ffcc00", fontsize=11, va="top", ha="left", family="monospace")
        ...
        self._txt_freq  = TextBox(ax_freq, "Freq MHz ",
                                  initial=f"{nco_hz/1e6:.4f}",
                                  color="#001155", hovercolor="#0a1a55",
                                  textalignment="center")
    ...
    def _toggle_overlay(self, _event):
        ...
        if not self._overlay:
            self.ax.set_title("Hermes Lite 2")
        self.fig.canvas.draw_idle()
    ...
    def _on_click(self, event):
        ...
        if event.inaxes is self._txt_lna.ax:
            self._anim_pause()
            self.ax.set_title("Paused — editing LNA gain (Enter to apply) ...")
            self.fig.canvas.draw_idle()
        elif event.inaxes is self._txt_freq.ax:
            self._anim_pause()
            self.ax.set_title("Paused — editing carrier freq (Enter to apply) ...")
            self.fig.canvas.draw_idle()
        else:
            self._anim_resume()
    ...
    def _on_lna_edit(self, _event):
        ...
        self._anim_pause()
        self.ax.set_title("Paused — enter LNA gain (0–48 dB) ...")
        self.fig.canvas.draw_idle()
        try:
    ...
    def _on_capture_click(self, _event):
        ...
        if self._on_capture:
            self.ax.set_title("Capturing 100 ms -> capture.npz ...")
            self.fig.canvas.draw_idle()
            self._on_capture()
    ...
    def _on_freq_edit(self, _event):
        ...
        self._anim_pause()
        self.ax.set_title("Paused — enter carrier freq (MHz) ...")
        self.fig.canvas.draw_idle()
        try:
            parent = getattr(self.fig.canvas.manager, "window", None)
    ...
    def set_corr(self, mag, best_df, peak, noise, dropped=0, locked=False):
        """Called from the capture thread with the matched-filter output."""
        with self._lock:
            self._corr_mag     = np.asarray(mag)
            self._corr_df      = best_df
            self._corr_peak    = peak
            self._corr_noise   = noise
            self._corr_dropped = dropped
            self._corr_locked  = locked
    ...
    def _animate(self, _frame):
        ...
        # Forward-power readout (PEP watts + PA current + raw ADC for verification)
        self._pwr_text.set_text(
            f"PEP {self._pep_watts:5.2f} W   PA {self._pa_current_ma:4.0f} mA   "
            f"(Fwd ADC {self._fwd_peak_adc:4d} of {self.FWD_ADC_FULLSCALE})")

        # Matched-filter correlation (bottom plot, dB relative to noise floor).
        # Contiguous code-aligned capture keeps the peak at a stable lag.
        if self._corr_mag is not None:
            m    = self._corr_mag
            n    = len(m)
            nf   = self._corr_noise if self._corr_noise > 0 else 1.0
            m_db = 20.0 * np.log10(np.maximum(m, 1e-12) / nf)
            t_ms = np.arange(n) / self._fs * 1e3
            self.corr_line.set_data(t_ms, m_db)
            self.ax_corr.set_xlim(0, t_ms[-1])
            self.ax_corr.set_ylim(-6.0, max(10.0, float(m_db.max()) + 3.0))
            snr = 20.0 * np.log10(self._corr_peak / nf + 1e-12)
            drop_txt = f"   DROPS {self._corr_dropped}" if self._corr_dropped else ""
            state = "LOCK" if self._corr_locked else "SRCH"
            self.ax_corr.set_title(
                f"Matched filter [{state}]   peak/noise {snr:4.1f} dB   "
                f"offset {self._corr_df:+.0f} Hz{drop_txt}",
                color=("#ff6666" if self._corr_dropped else
                       "#66ff66" if self._corr_locked else "white"))
