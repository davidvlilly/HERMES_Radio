...
class RadioEngine:
    def __init__(self, ip=HL2_IP, nco_freq=NCO_FREQ, log_path=None):
        ...
        # Spectrum rolling buffer + listen accumulator
        self._spec_buf  = []          # recent (I,Q) floats for the spectrum
    ...
    def end_mode(self):
        """Operator-requested end: send two end bursts, then drop."""
        with self._lock:
            if self._mode is None:
                return
            self._pending_end = 2
        self.emit("status", "Ending connection (sending end bursts) …")
    ...
    def _tx_thread(self):
        ...
        while self._running.is_set():
            with self._tx_lock:
                st = self._tx_state
                if st["active"] and st["pos"] < len(st["iq"]):
    ...
    def _demod_worker(self):
        while self._running.is_set():
            try:
                kind, partner, iq, accum_start = self._demod_q.get(timeout=0.2)
            except queue.Empty:
                continue
            try:
                acq, dec = self.modem.receive(iq)
    ...
    def _engine_thread(self):
        ...
        while self._running.is_set():
            ...
            # 3) Spectrum push (frozen while transmitting — holds last RX view).
            now = time.monotonic()
            if (not self._ptt and now - last_spec >= SPEC_PERIOD_S
                    and len(self._spec_buf) >= SPEC_SEG):
                freqs, power = compute_spectrum(self._spec_buf[-SPEC_PAD * 2:],
                                                self._nco_hz, SAMPLE_RATE)
                if freqs is not None:
                    self.emit("spectrum", freqs, power)
                last_spec = now

            # 4) Dispatch any completed demods, then run the scheduler.
            self._drain_results()
            mode = self._mode
            if mode == "call":
                self._tick_caller()
    ...
    def _ingest(self, data, accumulate):
        ...
        # Only feed the spectrum from received samples — during TX the RX stream
        # isn't the band we're listening to, so the display freezes (see below).
        if not self._ptt:
            self._spec_buf.extend(samp)
            if len(self._spec_buf) > SPEC_PAD * 4:
                self._spec_buf = self._spec_buf[-SPEC_PAD * 4:]
    ...
    def _tick_responder(self, prev_slot):
        ...
        if self._cycle_anchor is None:
            ...
            if (not self._demod_busy and not self._search_pending and
                ...
                self._demod_q.put(("search", "caller", iq, sample0))
            return prev_slot
    ...
    def _process_listen(self, partner):
        ...
        self._rx_accum = []
        self._demod_busy = True
        self._demod_q.put(("listen", partner, iq, sample0))
    ...
    def _register_miss(self, partner):
        ...
        if self._miss >= MAX_MISS:
            ...
            self._drop_mode("Connection lost (3 missed bursts)")
    ...
    def _handle_burst(self, dec, acq, partner):
        ...
        if dec["n_payload"] == 0:
            ...
            self.emit("rx_frag")         # red '*' — a message fragment is arriving
    ...
    def _begin_my_burst(self, role):
        ...
        if payload:
            ...
            # terminating newline marks the end of the message).
