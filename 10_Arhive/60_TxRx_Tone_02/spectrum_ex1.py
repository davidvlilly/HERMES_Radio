...
class LiveSpectrum:
    ...
    def __init__(self, nco_hz, fs, dbfs_ref=0.0,
        ...
        self.peak_line = self.ax.axvline(nco_hz / 1e6, color="#ff4444",
                                         ...
                                         visible=False)
        # RBW as plain text (upper-right), in Hz — not a legend.
        self._rbw_text = self.ax.text(
            0.99, 0.98, f"RBW {rbw_hz:.0f} Hz",
        ...
        # (seg, pad) chosen so RBW ≈ 1000 / 100 / 10 Hz within CAPTURE_SIZE.
        self._rbw_options = [("17 Hz",   32768, 32768),   # ≈ 10 Hz  (3 avgs)
                             ("135 Hz",   4096,  8192),   # ≈ 100 Hz (~31 avgs)
                             ("1080 Hz",   512,  2048)]   # ≈ 1 kHz  (many avgs)
        active = next((i for i, o in enumerate(self._rbw_options)
                       if o[1] == self._seg), 0)
        self._rbw_label = self.fig.text(0.762, 0.165, "RBW", color="white",
                                        fontsize=8, family="monospace")
        ax_rbw = self.fig.add_axes([0.80, 0.095, 0.10, 0.075])
    ...
    def _on_rbw_select(self, label):
        ...
        for lab, seg, pad in self._rbw_options:
            if lab == label:
                self._seg = seg
                self._pad = pad
                self._rbw_text.set_text(f"RBW {(self._fs / seg) * 1.44:.0f} Hz")
                with self._lock:
    ...
    def _animate(self, _frame):
        ...
        # Forward-power readout — PEP watts only
        self._pwr_text.set_text(f"pep {self._pep_watts:.1f}w")
