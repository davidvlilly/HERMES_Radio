...
def main():
    ...
    ap.add_argument("--sim-snr", type=float, default=None,
                    help="simulated channel SNR in dB (implies --sim)")
    args = ap.parse_args()

    # Giving --sim-snr alone means you want the simulation.
    if args.sim_snr is not None and not (args.simtest or args.selftest):
        args.sim = True
    snr = args.sim_snr if args.sim_snr is not None else -8.0
