...
class DbpskModem:
    def __init__(self, sample_rate=96_000, baud=40, pedestal_hz=2000.0,
                 ...
                 freq_search_hz=800.0, freq_step_hz=0.5, lpf_cutoff_hz=900.0):
        # WIDE-SEARCH test setting: +/-800 Hz carrier search with a 900 Hz baseband
        # lowpass (so large oscillator offsets pass through to be found). Max
        # offset is bounded by the 2 kHz baseband Nyquist (~+/-900 Hz); for more,
        # reduce `decim`. Normal/efficient values are ~255 Hz search / 400 Hz LPF.
