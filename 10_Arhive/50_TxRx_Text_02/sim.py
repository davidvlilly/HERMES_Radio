...
class SimMedium:
    def __init__(self, offset_hz=37.0, snr_db=-8.0, amplitude=eng.TX_AMPLITUDE):
        ...
        # Per-component RX noise. Low enough that the SAME spectrum calibration
        # (DBFS_REF = 8) reads a realistic ~-90 dBm floor, yet ~100x the 24-bit
        # LSB so we stay thermal-limited (decode unaffected — no quantization
        # limiting). Signal scales with it, so the chosen SNR is preserved.
        self.noise_std = 4.1e-4
        snr_lin = 10 ** (snr_db / 10.0)
        a_rx = np.sqrt(snr_lin * 2.0 * self.noise_std ** 2 * 2500.0 / FS)
        a_rx = min(a_rx, 0.6)                  # cap so a strong signal still fits
