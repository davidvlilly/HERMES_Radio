...
class RadioGUI:
    ...
    def _build_log(self):
        ...
        tk.Button(compose, text="Clear", command=self._on_clear).pack(
            side=tk.LEFT, padx=4)
        self.cap_btn = tk.Button(compose, text="Capture", command=self._on_capture)
        self.cap_btn.pack(side=tk.LEFT, padx=4)
        self._cap_btn_bg = self.cap_btn.cget("background")

        self.tone_btn = tk.Button(compose, text="Tone", command=self._on_tone)
        self.tone_btn.pack(side=tk.LEFT, padx=4)
        self._tone_btn_bg = self.tone_btn.cget("background")
        self._tone_on = False

        self.inv_btn = tk.Button(compose, text="InvFrq", command=self._on_invert)
        self.inv_btn.pack(side=tk.LEFT, padx=4)
        self._inv_btn_bg = self.inv_btn.cget("background")
        self._inv_on = False
    ...
    def _on_capture(self):
        self.engine.request_capture()      # ~6 s of raw RX -> capture.npz

    def _on_tone(self):
        self.engine.set_tone(not self._tone_on)   # toggle CW carrier

    def _on_invert(self):
        self.engine.set_invert(not self._inv_on)  # toggle RX I/Q inversion
    ...
    def _handle_event(self, ev):
        ...
        elif kind == "note":                            # capture start/done, etc.
            self._left_line(ev[1])
        elif kind == "capturing":                       # live RX-record indicator
            if ev[1]:
                self.cap_btn.config(text="● REC", fg="white", bg="#cc3333")
            else:
                self.cap_btn.config(text="Capture", fg="black", bg=self._cap_btn_bg)
        elif kind == "tone":                            # CW-carrier toggle state
            self._tone_on = bool(ev[1])
            if self._tone_on:
                self.tone_btn.config(text="● TONE", fg="white", bg="#cc3333")
            else:
                self.tone_btn.config(text="Tone", fg="black", bg=self._tone_btn_bg)
        elif kind == "invert":                          # RX I/Q invert toggle state
            self._inv_on = bool(ev[1])
            if self._inv_on:
                self.inv_btn.config(text="InvFrq*", fg="white", bg="#3377cc")
            else:
                self.inv_btn.config(text="InvFrq", fg="black", bg=self._inv_btn_bg)
        elif kind == "mode":
            name = ev[1]
